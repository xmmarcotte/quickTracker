...
def splash():
    ...
    customtkinter.CTkLabel(master=main, text='Please wait while the application loads...').pack()
...
def getCreds():
    ...
    customtkinter.CTkButton(main_frame,
                            text='Submit',
                            command=printValue2).pack(pady=(0, 10))
...
def login():
    ...
    try:
        try:
            connection = pymssql.connect(server='gp2018', user=f'GRT0\\{user_name}', password=password,
                                         database='SBM01')
            if not os.path.exists("credentials.txt"):
                encrypted_un = encrypt(user_name)
                encrypted_pass = encrypt(password)
                fh = open('credentials.txt', 'w')
...
def main():
    ...
    main.attributes('-alpha', 0.95)
    center_win(main)
    customtkinter.CTkLabel(master=main_frame, text='Please enter the ticket number(s):').pack(pady=(20, 0))
...
def main2():
    ...
    greeting = customtkinter.CTkLabel(master=main2_frame, text='Loading results...', width=200, height=100)
...
Created by Mikey Marcotte"""
    ...
    insert_initial()
    customtkinter.CTkButton(master=win_frame,
                            text='Clear',
                            width=100,
                            command=clear).pack(side='left', anchor='ne', expand=True, padx=5, pady=(0, 10))
    customtkinter.CTkButton(master=win_frame,
                            text='Exit',
                            width=100,
                            command=endProg).pack(side='right', anchor='nw', expand=True, padx=5, pady=(0, 10))
...
def popUp():
    ...
    for i in text_response:
        text.insert(END, str(i) + "\n")
    text.pack(padx=10, pady=10)
    customtkinter.CTkButton(win_frame,
                            text='Start Over',
                            width=100,
                            command=restart).pack(side='left', anchor='ne', expand=True, padx=5, pady=(0, 10))
    customtkinter.CTkButton(win_frame,
                            text='Exit',
                            width=100,
                            command=endProg).pack(side='right', anchor='nw', expand=True, padx=5, pady=(0, 10))
...
def track_tickets():
    ...
    for tkt in tkt_list:
        tkt = tkt.strip()
        cursor.execute(f"""declare @ticket as varchar(100) = '{tkt}'
    ...
    or (sop10107.SOPNUMBE = @ticket or sop10107.SOPNUMBE = 'CW' + @ticket + '-1'))""")
        ...
        if inquiry_number:
            ...
            if shipper == 'UPS':
                ...
                try:
                    status = json.loads(
                        tracking_info)['trackResponse']['shipment'][0]['package'][0]['activity'][0]['status'][
                        'description'].strip()
                    try:
                        del_date = \
                            json.loads(tracking_info)['trackResponse']['shipment'][0]['package'][0]['deliveryDate'][
                                0][
                                'date']
                        ...
                        del_time = \
                            json.loads(tracking_info)['trackResponse']['shipment'][0]['package'][0]['deliveryTime'][
                                'type'].strip()
                        if del_time == "CMT":
                            del_time = "AM"
                    ...
                    datestr = False
                    status = "We could not locate the shipment details for this tracking number. Details are only " \
                ...
                status = json.loads(response.content)['output']['completeTrackResults'][0]['trackResults'][0][
                    'latestStatusDetail']['statusByLocale']
                del_date = \
                    json.loads(response.content)['output']['completeTrackResults'][0]['trackResults'][0][
                        'dateAndTimes'][0][
                        'dateTime']
            ...
            try:
                ...
                if status == 'RDY TO INVOICE':
                    status = status + " - No tracking available. This may be a partial shipment. Tracking may be " \
